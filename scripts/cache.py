...
"""Cache administration — an inspection window, not the cache itself.
    ...
    ./cache.py                       # stats for both caches
...
Two caches share one SQLite file because their lifetimes differ:
...
page throws away ~1000 cards that cost ~14 Scryfall round trips to rebuild.
...
import toollog
import workspace
from cardlib import CardQuery, PageStore


def main() -> int:
    toollog.record()
    ap = argparse.ArgumentParser(
        description="Inspect and clear the Scryfall card cache and EDHREC page cache.")
    ap.add_argument("--stats", action="store_true")
    ap.add_argument("--clear", nargs="?", const="all",
                    choices=["all", "cards", "pages"], metavar="WHAT",
                    help="wipe all|cards|pages (default: all)")
    ...
    if args.stats or not any([args.clear, args.warm]):
        for k, v in {**q.stats(), **pages.stats()}.items():
            print(f"{k:>22}: {v}")
    return 0
